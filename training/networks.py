...
class AMED_predictor(torch.nn.Module):
    ...
    def forward(self, unet_bottleneck, t_cur, t_next, class_labels=None):
        ...
        r = self.sigmoid(self.fc_r(out))
        if self.scale_dir:
            scale_dir = self.sigmoid(self.fc_scale_dir(out)) / (1 / (2 * self.scale_dir)) + (1 - self.scale_dir)
            if not self.scale_time:
                return r, scale_dir
        if self.scale_time:
            scale_time = self.sigmoid(self.fc_scale_time(out)) / (1 / (2 * self.scale_time)) + (1 - self.scale_time)
